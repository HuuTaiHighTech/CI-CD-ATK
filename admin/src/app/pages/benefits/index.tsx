...
import { BenefitTable } from '~/components/tables';
...
import { BenefitDialog } from '~/components/dialogs';
import SelectPopover from '~/components/ui/select-popover';

function BenefitPage() {
   const { set, items, pagination, setItem, isEmpty } = usePaginated<Benefit>();
   ...
   const fetchBenefit = useCallback(async () => {
      try {
         setIsLoading(true);
         const { data } = await benefitService.get({
            page,
            limit,
            search: stableSearch,
            visible
   ...
   const handleToggleVisible = useCallback(
      async (id: string, visible: boolean) => {
         try {
            setIsLoading(true);
            await benefitService.upSert(id, { visible });
   ...
   const handleDelete = useCallback(
      ...
      [items.length, page, setPage, fetchBenefit]
   );
            ...
            <div className='flex flex-col sm:flex-row justify-between items-center gap-3 mb-6'>
               <div>
                  ...
                  <p className='text-sm md:text-base text-muted-foreground'>
                     Quản lý danh sách lợi ích của bạn
                  </p>
               </div>
               <Button
