...
import { SocialDialog } from '~/components/dialogs';
import { SocialTable } from '~/components/tables';
...
function SocialPage() {
   const { set, items, pagination, setItem, isEmpty } = usePaginated<Social>();
   ...
   const handleEdit = useCallback((data: Social) => {
      setSocial(data);
      setIsOpen(true);
   }, []);
   ...
   const handleToggle = useCallback(
      async (id: string, visible: boolean) => {
         try {
            setIsLoading(true);
            await socialService.upSert(id, { visible });
            ...
            <div className='flex flex-col sm:flex-row justify-between items-center gap-3 mb-6'>
               <div>
                  ...
                  <p className='text-sm md:text-base text-muted-foreground'>
                     Quản lý danh sách mạng xã hội của bạn
                  </p>
               </div>
